...
class __CompletionDaemon(Thread):
    ...
    def _resolve_with_other_modules(self, packages):
        ...
        for package in packages:
            ...
            elif self.projects_modules.get(package.rsplit(b'.', 1)[0], False):
                name = package.rsplit(b'.', 1)
                folder = self.projects_modules[name[0]]
                filename = "%s.py" % os.path.join(folder, name[1])
                if os.path.isfile(filename):
                    if self._analyze_file(filename):
                        resolution[package] = filename
            elif self.projects_modules.get(package.rsplit(b'.', 2)[0], False):
                name = package.rsplit(b'.', 2)
                folder = self.projects_modules[name[0]]
                filename = "%s.py" % os.path.join(folder, name[1])
                if os.path.isfile(filename):
                    if self._analyze_file(filename):
                        resolution[package.rsplit(b'.', 1)[0]] = filename
        return resolution
...
class _DaemonProcess(Process):
        ...
        for data in types:
            ...
            for package in self.packages:
                if name.startswith(package):
                    ...
                    if to_resolve and to_resolve[0] == b'.':
    ...
    def _resolve_with_imports(self, types, module, splitby):
        for data in types:
            ...
            value = line.split(bytes(splitby, encoding="utf-8", errors="ignore"))[1].strip().split(b'.')
            name = value[0]
            extra = ''
            if name.find(b'(') != -1:
                extra = name[name.index(b'('):]
                name = name[:name.index(b'(')]
            if name in module.imports:
                value[0] = module.imports[name].data_type
                package = b'.'.join(value)
    ...
    def _resolve_with_local_names(self, types, module, splitby):
        ...
        for data in types:
            if data.data_type != model.late_resolution:
                continue
            line = data.line_content
            value = line.split(bytes(splitby, encoding="utf-8", errors="ignore"))[1].split(b'(')[0].strip()
    ...
    def _resolve_with_local_vars(self, types, module, splitby, structure=None):
        for data in types:
            ...
            if len(sym) != 0:
                ...
                if isinstance(result['type'], str) and len(result) < 3:
                    if child_attr and \
