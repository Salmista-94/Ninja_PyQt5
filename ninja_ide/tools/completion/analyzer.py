        ...
        try:
            ...
            if line != self._fixed_line and reason.text is not None:
                ...
                if isinstance(split_source, str):
                    # source = '\n'.join(split_source)
                    try:
                        split_source = bytes(split_source, encoding="utf-8", errors="ignore")
...
#        self._functions = {}
    ...
    def _assign_disambiguation(self, type_name, line_content):
        ...
        if type_name is _ast.Num and b'.' in value:
            type_name = b'_ast.Float'
        elif value in (b'True', b'False'):
            type_name = b'_ast.Bool'
        elif value == b'None':
            type_name = None
        return type_name
...
#            function.decorators.append(decorator.id)
        ...
        for arg in reversed(symbol.args.args):
            if isinstance(arg, ast.Tuple):
                ...
                continue
            elif arg.arg == 'self':#arg.id
                continue
            assign = model.Assign(arg.arg)#arg.id
    ...
    def _search_recursive_for_types(self, function, symbol, parent=None):
        ...
        elif symbol.__class__ is ast.Try:#TryFinally
            for sym in symbol.body:
                self._search_recursive_for_types(function, sym, parent)
