...
def remove_function_arguments(line):
    ...
    if isinstance(line, str):
        line = bytes(line, encoding="utf-8", errors="ignore")
    while line.find(b'(') != -1:
        start = line.find(b'(')
        end = line.find(b')') + 1
    ...
    def get_type(self, main_attr, child_attrs='', scope=None):
        ...
        if not scope:
            ...
            if structure is not None:
                attrs = [main_attr] + canonical_attrs.split(b'.')
        ...
        if result['type'].__class__ is Clazz:
            ...
            if main_attr == 'self':
                attrs = canonical_attrs.split(b'.', 1)
                canonical_attrs = ''
                if len(attrs) > 1:
...
class LinkedModule(object):
    ...
    def get_type(self, resolve=''):
        ...
        if module:
            ...
            to_resolve = to_resolve.split(b'.', 1)
