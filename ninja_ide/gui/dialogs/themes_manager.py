...
class ThemesManagerWidget(QDialog):
    ...
    def __init__(self, parent):
        super(ThemesManagerWidget, self).__init__(parent, Qt.Dialog)
        self.setWindowTitle(_translate("ThemesManagerWidget", "Themes Manager"))
        self.resize(700, 500)
        ...
        # Footer
        hbox = QHBoxLayout()
        btn_close = QPushButton(_translate("ThemesManagerWidget", 'Close'))
        btnReload = QPushButton(_translate("ThemesManagerWidget", "Reload"))
    ...
    def load_skins_data(self):
        if self._loading:
            ...
            self._tabs.addTab(self._schemeWidget, _translate("ThemesManagerWidget", "Editor Schemes"))
            self._loading = False
        self.overlay.hide()
        self._thread.wait()
...
class SchemeWidget(QWidget):
    ...
    def __init__(self, parent, schemes):
        super(SchemeWidget, self).__init__(parent)
        self._parent = parent
        self._schemes = schemes
        vbox = QVBoxLayout(self)
        ...
        ui_tools.load_table(self._table, [_translate("SchemeWidget", 'Name'), _translate("SchemeWidget", 'URL')],
            self._schemes)
        btnUninstall = QPushButton(_translate("SchemeWidget", 'Download'))
