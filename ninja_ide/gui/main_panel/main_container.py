...
def MainContainer(*args, **kw):
    ...
    if __mainContainerInstance is None:
        __mainContainerInstance = _s_MainContainer(*args, **kw)
    return __mainContainerInstance


class _s_MainContainer(QSplitter):
...
###############################################################################
    ...
    def _cursor_position_changed(self, row, col):
        self.cursorPositionChange.emit(row, col)
    ...
    def close_deleted_file(self, id):
        ...
        if index != -1:
            ...
            index = self._tabSecondary.is_open(id)
            tabContainer = self._tabSecondary
        result = QMessageBox.question(self, _translate("_s_MainContainer", "Close Deleted File"),
            _translate("_s_MainContainer", "Are you sure you want to close the deleted file?\n"
    ...
    def show_start_page(self):
        if not self.is_open("Start Page"):
            ...
            startPage.openProject[str].connect(self.open_project)
    ...
    def show_python_doc(self):
        if sys.platform == 'win32':
            ...
            self.add_tab(self.docPage, _translate("_s_MainContainer", "Python Documentation"))
        else:
