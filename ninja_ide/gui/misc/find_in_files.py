...
class FindInFilesThread(QThread):
    ...
    '''
    found_pattern = pyqtSignal(tuple)
...
class FindInFilesResult(QTreeWidget):
    ...
    def __init__(self):
        ...
        self.setHeaderLabels((_translate("FindInFilesResult", 'File'), _translate("FindInFilesResult", 'Line')))
        ...
        self.header().setSectionResizeMode(1, QHeaderView.ResizeToContents)#self.header().setResizeMode(1, QHeaderView.ResizeToContents)
...
class FindInFilesDialog(QDialog):
    ...
    def __init__(self, result_widget, parent):
        ...
        self.open_button = QPushButton(QIcon(resources.IMAGES['find']),
            _translate("FindInFilesDialog", "Open"))
        self.filters_line_edit = QLineEdit("*.py")
        self.replace_line = QLineEdit()
        self.replace_line.setEnabled(False)
        self.check_replace = QCheckBox(_translate("FindInFilesDialog", "Replace: "))
        self.case_checkbox = QCheckBox(_translate("FindInFilesDialog", "C&ase sensitive"))
        self.type_checkbox = QCheckBox(_translate("FindInFilesDialog", "R&egular Expression"))
        self.recursive_checkbox = QCheckBox(_translate("FindInFilesDialog", "Rec&ursive"))
        self.recursive_checkbox.setCheckState(Qt.Checked)
        self.phrase_radio = QRadioButton(
            _translate("FindInFilesDialog", "Search by Phrase (Exact Match)."))
        self.phrase_radio.setChecked(True)
        self.words_radio = QRadioButton(
            _translate("FindInFilesDialog", "Search for all the words "
                    "(anywhere in the document, not together)."))
        self.find_button = QPushButton(_translate("FindInFilesDialog", "Find!"))
        self.find_button.setMaximumWidth(150)
        self.cancel_button = QPushButton(_translate("FindInFilesDialog", "Cancel"))
        ...
        #main section
        find_group_box = QGroupBox(_translate("FindInFilesDialog", "Main"))
        grid = QGridLayout()
        grid.addWidget(QLabel(_translate("FindInFilesDialog", "Text: ")), 0, 0)
        grid.addWidget(self.pattern_line_edit, 0, 1)
        grid.addWidget(QLabel(_translate("FindInFilesDialog", "Directory: ")), 1, 0)
        grid.addWidget(self.dir_combo, 1, 1)
        grid.addWidget(self.open_button, 1, 2)
        grid.addWidget(QLabel(_translate("FindInFilesDialog", "Filter: ")), 2, 0)
        ...
        #options sections
        options_group_box = QGroupBox(_translate("FindInFilesDialog", "Options"))
    ...
    def _select_dir(self):
        dir_name = QFileDialog.getExistingDirectory(self,
            _translate("FindInFilesDialog", "Open Directory"),
    ...
    def _found_match(self, result):
        #file_name = result[0]
        #items = result[1]
        ##file_name, items = result
        self.result_widget.update_result(
            self.dir_combo.currentText(), *result)#self.dir_combo.currentText(), file_name, items)
...
class FindInFilesWidget(QWidget):
    ...
    def __init__(self, parent):
        ...
        self._open_find_button = QPushButton(_translate("FindInFilesWidget", "Find!"))
        self._stop_button = QPushButton(_translate("FindInFilesWidget", "Stop"))
        self._clear_button = QPushButton(_translate("FindInFilesWidget", "Clear!"))
        self._replace_button = QPushButton(_translate("FindInFilesWidget", "Replace"))
        self._find_widget = FindInFilesDialog(self._result_widget, self)
        self._error_label = QLabel(_translate("FindInFilesWidget", "No Results"))
        self._error_label.setVisible(False)
        #Replace Area
        self.replace_widget = QWidget()
        hbox_replace = QHBoxLayout(self.replace_widget)
        hbox_replace.setContentsMargins(0, 0, 0, 0)
        self.lbl_replace = QLabel(_translate("FindInFilesWidget", "Replace results with:"))
    ...
    def _replace_results(self):
        result = QMessageBox.question(self, _translate("FindInFilesWidget", "Replace Files Contents"),
            _translate("FindInFilesWidget", "Are you sure you want to replace the content in "
