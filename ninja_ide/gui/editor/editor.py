...
###############################################################################
    ...
    def __init__(self, filename, project, project_obj=None):
        ...
        self.updateRequest['const QRect&', int].connect(self._sidebarWidget.update_area)
        self.undoAvailable['bool'].connect(self._file_saved)
        ...
        if settings.SHOW_MINIMAP:
            ...
            self._mini.show()
            self.updateRequest['const QRect&', int].connect(self._mini.update_visible_area)
            
        #Indentation
        self.set_project(project_obj)
        #Context Menu Options
        self.__actionFindOccurrences = QAction(
            _translate("Editor", "Find Usages"), self)
    ...
    def set_project(self, project):
        if project is not None:
            ...
            if self.useTabs:
                self.set_tab_usage()
            project._parent.projectPropertiesUpdated['QTreeWidgetItem*'].connect(self.set_project)
    ...
    def jump_to_line(self, lineno=None):
        ...
        line = QInputDialog.getInt(self, _translate("Editor", "Jump to Line"),
            _translate("Editor", "Line:"), 1, 1, maximum, 1)
        if line[1]:
            self.addBackItemNavigation.emit()
    ...
    def indent_more(self):
        ...
        end = next(self.document().findBlock(cursor.selectionEnd()))
    ...
    def contextMenuEvent(self, event):
        ...
        menu_lint = QMenu(_translate("Editor", "Ignore Lint"))
        ignoreLineAction = menu_lint.addAction(
            _translate("Editor", "Ignore This Line"))
        ignoreSelectedAction = menu_lint.addAction(
            _translate("Editor", "Ignore Selected Area"))
        ignoreLineAction.triggered.connect(lambda: helpers.lint_ignore_line(self))
        ignoreSelectedAction.triggered.connect(lambda: helpers.lint_ignore_selection(self))
    ...
    def dropEvent(self, event):
        if len(event.mimeData().urls()) > 0:
            print("\nevent.mimeData().urls()::", event.mimeData(), event.mimeData().urls()[0],
                event.mimeData().urls()[0].toLocalFile()[:4],
                event.mimeData().urls()[0].toLocalFile()[-4:])
            path = event.mimeData().urls()[0].toLocalFile()#.path()
    ...
    def highlight_current_line(self):
        self.cursorPositionChange.emit(
