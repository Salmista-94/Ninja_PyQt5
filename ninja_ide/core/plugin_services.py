...
class MainService(QObject):
    ...
    def __init__(self):
        ...
        #Connect signals
        self._main.editorKeyPressEvent['QEvent*'].connect(self._keyPressEvent)
        self._main.beforeFileSaved[str].connect(self._beforeFileSaved)
        self._main.fileSaved[str].connect(self._fileSaved)
        self._main.currentTabChanged[str].connect(self._currentTabChanged)
        self._action.fileExecuted[str].connect(self._fileExecuted)
        self._main.fileOpened[str].connect(self._fileOpened)
...
class MiscContainerService(QObject):
    ...
    def __init__(self, miscContainer):
        super(MiscContainerService, self).__init__()
...
class ExplorerService(QObject):
    ...
    def __init__(self):
        ...
        self._explorer = explorer_container.ExplorerContainer()
        self._action = actions.Actions()
        self._explorer.projectOpened[str].connect(self._projectOpened)
        self._action.projectExecuted[str].connect(self._projectExecuted)
